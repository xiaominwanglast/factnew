...
class CarDecisionApiAuto(DecisionApi,CarDecisionProperty):
    ...
    def makeDecision(self, certId='', phone='', userName='', bankcardNo='', registerLocation='', agentName='',**dictargs):
        """
        ...
        :return:
        """
        # IfIsInstance = lambda Info: Info if not isinstance(Info, str) else Info.encode('utf-8')
        ...
        apply_data = {"certId": certId, "phone": phone, "productId": self.productId, "sceneId": self.sceneId,
                      ...
                      "czcbGeneralRiskResult":self.czcbGeneralRiskResult,"czcbDebtRiskResult":self.czcbDebtRiskResult,
                      "czcbOverdueRiskResult":self.czcbOverdueRiskResult,"czcbRiskNote":self.czcbRiskNote,
                      ...
                      "pbocWhite":self.pbocWhite,"applicantNation":self.applicantNation,"lastAuditResult":self.lastAuditResult,
                      "gpsPrice":self.gpsPrice,"carPrice":self.carPrice,"carInsurance":self.carInsurance,"carPurchaseTax":self.carPurchaseTax,"carProductType":self.carProductType,
                      "cdwMakeLoanAmount":self.cdwMakeLoanAmount,"carZoneCode":self.carZoneCode,"precallRisklevel":self.precallRisklevel,"carPurchase":self.carPurchase,
                      "bankPhone":self.bankPhone,"colleagueContact":self.colleagueContact,"friendContact":self.friendContact,"relativeContact":self.relativeContact,
                      "firstApplyTime":self.firstApplyTime,"livingAddress":self.livingAddress,"residenceAddress":self.residenceAddress,
                      "unitAddress":self.unitAddress,"unitName":self.unitName
                      }
        #TODO 用传入的字典来更新传入参数
        apply_data = apply_data.copy()
        apply_data.update(dictargs['test_data'])
        #TODO 不加回调函数
        requestSign=""
        applykeys=list(apply_data.keys())
        applykeys.sort()
        for key in applykeys:
            requestSign=requestSign+str(key)+'='+str(apply_data.get(key))+'&'
        requestSign=requestSign+self.salt
        m = hashlib.md5()
        m.update(requestSign.encode('utf-8'))
...
if __name__ == '__main__':
    env = 'T1'
    test = CarDecisionApiAuto(env='T1', productId='CDW', sceneId='approval')
    print (test.makeDecision(certId='371312198808046937', phone='13355495678', userName='王立吉',test_data={"loanProduct":'17'}))
