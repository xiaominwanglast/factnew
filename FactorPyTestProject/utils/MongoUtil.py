            ...
            if 's_user_mobile_contact_action' in collection:
                if data.get('is_gzip') and data.get('is_gzip') == 1:
                    if data.get('content'):
                        ...
                        del data['content']
                        for action in dump:
                            if isinstance(action.get('callTime'), int):
...
class MongoPyUtil(object):
    def __init__(self, env,db):
        self.db=db
        self.client= pymongo.MongoClient(SETTING[env]['MONGO'][self.db])
        self.database = self.client[self.db.split('_')[0]]

    def __del__(self):
        self.client.close()

    #查询最新一条
    def queryNewOne(self,collection,find):
        ret=self.database[collection].find(find).sort("_id", -1).limit(1)
        ret=self.public_get_gzip_data(ret,collection)
        return ret

    #查询全部信息
    def queryAll(self,collection,find):
        ret=self.database[collection].find(find).sort("_id", -1)
        ret=self.public_get_gzip_data(ret,collection)
        return ret
    ...
    #更新一条数据
    def updateOneData(self,collection,query,update):
        self.database[collection].update_one(query,update)

    #全部删除
    def deleteAllData(self,collection):
            ...
            if 's_user_mobile_contact_action' in collection:
                if data.get('is_gzip') and data.get('is_gzip') == 1:
                    if data.get('content'):
                        ...
                        for action in dump:
                            if isinstance(action.get('callTime'), int):
                                action['callTime'] = datetime.datetime.strptime(time.strftime('%Y-%m-%d %H:%M:%S',
                                                                                              time.localtime(
                                                                                                  action.get(
                                                                                                      'callTime') / 1000)),
                                                                                '%Y-%m-%d %H:%M:%S')
                    ...
                    new_dataList.append(data)
            if collection in ["mx_alipay_bills_data", "mx_alipay_report", "mx_carrier_bills_notify",
                              "mx_carrier_report_notify", "mx_carrier_smses", "mx_carrier_nets", "mx_zhengxin_report",
                              "mx_email_bills", "mx_email_report"]:
                if data.get('is_gzip') and int(data.get('is_gzip')) == 1:
                    if data.get('content'):
                        dump = Des().unGzip(data.get('content'))
                        if isinstance(dump, str):
                            dump = json.loads(dump)
...
class MongoPyUtilPort(object):
    def __init__(self, env,db):
        self.env=env
        self.db=db
    def deleteData(self,collection,dataDict):
        try:
            MongoPyUtil(env=self.env, db=self.db+'_27018').deleteData(collection=collection, data=dataDict)
        except:
            MongoPyUtil(env=self.env, db=self.db + '_27017').deleteData(collection=collection, data=dataDict)

    def deleteAllData(self,collection):
        try:
            MongoPyUtil(env=self.env, db=self.db+'_27018').deleteAllData(collection=collection)
        except:
            MongoPyUtil(env=self.env, db=self.db + '_27017').deleteAllData(collection=collection)

    def insertData(self,collection,dataList):
        try:
            MongoPyUtil(env=self.env, db=self.db+'_27017').insertData(collection=collection, data=dataList)
        except:
            MongoPyUtil(env=self.env, db=self.db + '_27018').insertData(collection=collection, data=dataList)

    def updateOne(self,collection,query,update):
        try:
            MongoPyUtil(env=self.env, db=self.db+'_27017').updateOneData(collection=collection, query=query,update=update)
        except:
            MongoPyUtil(env=self.env, db=self.db + '_27018').updateOneData(collection=collection,query=query,update=update)

if __name__=="__main__":
    # MongoPyUtilPort(env='T1', db='galaxy').deleteData(collection='cdw_city_black_record', dataDict={'city': '内蒙古'})
    # MongoPyUtilPort(env='T1', db='galaxy').insertData(collection='cdw_city_black_record', dataList=[{'city': '内蒙古'}])
    MongoPyUtilPort(env='T1', db='skynet').updateOne(collection='skynet_decision_mock',query={"_id" : "CDW_approval"},update={'$set':{"valid":False}})
    # MongoPyUtilPort(env='T1', db='galaxy').insertData(collection='cdw_dealer_white_record', dataList=[{'dealer': '1000', 'dealerName': '1000', 'dealerType': 'D', 'isDelete': 0 }])
    # MongoPyUtil(env='T1', db='galaxy_27018').insertData(collection='cdw_city_black_record',data=[{'city': '内蒙古'}])
    # MongoPyUtil(env='T1', db='galaxy_27018').deleteData(collection='cdw_city_black_record',data={'city': '内蒙古'})
