...
import re,json,time
from .threeDES import factor_decrypt_identity
...
class InfoHelper(object):
    def __init__(self, env, serial_no, mongdb='skynet'):
        ...
        data=json.loads(factor_decrypt_identity(self.result.get('encryption_data')))
        self.result['data']=data
        if data:
            self.old_user_id = data.get('oldUserId')
            self.colleagueContact = data.get('colleagueContact')
            self.relativeContact = data.get('relativeContact')
            self.friendContact = data.get('friendContact')
            self.homeAddress = data.get('homeAddress')
            self.unitsName = data.get('unitsName')
            self.unitsAddress= data.get('unitsAddress')
